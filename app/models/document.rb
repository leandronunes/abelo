class Document < ActiveRecord::Base
  ...
  def validate
    ...
    self.errors.add(:owner, t(:you_cannot_have_a_non_document_model_whitout_a_customer_supplier_or_worker_associated_to_him)) if !self.is_model? and (self.owner_type.nil? or self.owner_id.nil?)
  end

  def validate_on_destroy
    if self.is_model? and !self.organization.nil? and self.organization.documents_by_model(self).size > 0
      self.errors.add( t(:you_can_not_destroy_this_model_exist_at_least_a_document_that_references_it)) if self.is_model? and self.organization.documents_by_model(self).size > 0
    end
  end
