...
#  has_one :change, :as => :owner, :class_name => Ledger.name, :foreign_key => :change_id
  ...
  def validate
    ...
    if !pending.nil? and pending != self
      self.errors.add(:status, I18n.t(:you_cannot_have_two_pendings_sale))
    end

    opened = Sale.opened(self.till)
    if !opened.nil? and opened != self
      self.errors.add(:status, I18n.t(:you_cannot_open_a_sale_with_a_sale_opened))
    end
  ...
  def cancel!
    if self.status != STATUS_PENDING
      self.errors.add('status', I18n.t(:only_open_sales_can_be_cancelled)) 
      return false
    end
    self.status = STATUS_CANCELLED
  ...
  def close!
    if self.status != STATUS_PENDING
      self.errors.add('status', I18n.t(:only_open_sales_can_be_closed)) 
      return false
    end

    if self.balance != 0
      self.errors.add('value', I18n.t(:only_sales_with_balance_equal_to_zero_can_be_closed)) 
      return false
    end
...
#      self.status = STATUS_OPEN
  ...
  def customer_description
    self.customer.nil? ? I18n.t(:none) :  self.customer.name
  end
