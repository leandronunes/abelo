class SaleItem < ActiveRecord::Base
  ...
  validates_associated :stock_out, :message => I18n.t(:insufficient_product_in_stock)
  ...
  def validate
    if self.unitary_price <= 0
      self.errors.add(:unitary_price, I18n.t(:the_price_must_be_greater_than_0))
    end
    ...
    if !self.sale.nil? and self.has_fiscal_printer? and (self.printer_command.nil? or !PrinterCommand.pending_command(self.till).nil?)
      self.errors.add(:printer_command, I18n.t(:you_have_pending_add_item_commands))
    end
  end
