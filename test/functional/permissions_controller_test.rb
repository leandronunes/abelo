...
class PermissionsControllerTest < Test::Unit::TestCase
  ...
  def test_index
    ...
    assert_kind_of Array, assigns(:user_profiles)
  ...
  def test_select_template
    ...
    assert_kind_of UserProfile, assigns(:user_profile)
  ...
  def test_update_template_success
    ...
    assert_equal 'read_only', UserProfile.find(3).template
    post :update_template, :id => 3, :user_profile => { :template => 'full_access' }
    assert_response :redirect
    assert_redirected_to :action => 'index'
    assert_equal 'full_access', UserProfile.find(3).template
  end
