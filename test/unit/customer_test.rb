...
class CustomerTest < Test::Unit::TestCase
  ...
  def test_relation_with_organization
    ...
    assert_equal @org, customer.organization
  end
  ...
  def test_mandatory_field_name
    customer = Customer.create(:email => 'teste@teste', :organization_id => 1, :category_id => @cat.id, :cpf => '23831442312')
    assert customer.errors.invalid?(:name)
  end

  def test_mandatory_field_organization_id
    customer = Customer.create(:name => 'new customer', :email => 'teste@teste', :category_id => @cat.id, :cpf => '23831442312')
  ...
  def test_mandatory_field_email
    customer = Customer.create(:name => 'new customer', :organization_id => 1, :category_id => @cat.id, :cpf => '23831442312')
    assert customer.errors.invalid?(:email)
  end

  def test_cnpj_valid_format
    customer = Customer.create(:name => 'customer', :email => 'teste@teste', :organization_id => 1, :category_id => @cat.id, :cnpj => '27122113000116')
    assert customer.errors.empty?
  end

  def test_cnpj_invalid_format
    customer = Customer.create(:name => 'customer', :email => 'teste@teste', :organization_id => 1, :category_id => @cat.id, :cnpj => '00000000000000')
    assert customer.errors.invalid?(:cnpj)
  end

  def test_cpf_valid_format
    customer = Customer.create(:name => 'customer', :email => 'teste@teste', :organization_id => 1, :category_id => @cat.id, :cpf => '86666532724')
    assert customer.errors.empty?
  end

  def test_cpf_invalid_format
    customer = Customer.create(:name => 'customer', :email => 'teste@teste', :organization_id => 1, :category_id => @cat.id, :cpf => '00000000000')
