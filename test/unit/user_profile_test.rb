...
class ProfileTest < Test::Unit::TestCase
  ...
  def test_allowed_actions_per_user
    ...
    assert(user1.allowed_to?(:organization_nickname => 'one', :controller => 'organization', :action => 'edit'))

    user2 = people(:dani)
    assert(!user2.allowed_to?(:organization_nickname => 'one', :controller => 'organization', :action => 'edit'))
  end
